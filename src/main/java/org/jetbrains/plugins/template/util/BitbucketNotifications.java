...
import static org.jetbrains.plugins.template.util.BitbucketUtil.getErrorTextFromException;

public class BitbucketNotifications {
  private static final Logger LOG = BitbucketUtil.LOG;

  private static boolean isOperationCanceled(@NotNull Throwable e) {
    return e instanceof BitbucketOperationCanceledException ||
  ...
  public static AnAction getConfigureAction(@NotNull Project project) {
    return NotificationAction.createSimple(GitBundle.messagePointer("action.NotificationAction.GithubNotifications.text.configure"),
                                           () -> ShowSettingsUtil.getInstance()
                                             .showSettingsDialog(project, BitbucketUtil.SERVICE_DISPLAY_NAME));
  }
}
