...
import org.jetbrains.plugins.template.BitbucketCreatePullRequestWorker;
...
import org.jetbrains.plugins.template.util.BitbucketProjectSettings;
...
import static org.jetbrains.plugins.template.BitbucketCreatePullRequestWorker.BranchInfo;
import static org.jetbrains.plugins.template.BitbucketCreatePullRequestWorker.ForkInfo;

/**
 * @author Aleksey Pivovarov
 */
public class BitbucketCreatePullRequestDialog extends DialogWrapper {
  @NotNull private final org.jetbrains.plugins.template.ui.GithubCreatePullRequestPanel myPanel;
  @NotNull private final BitbucketCreatePullRequestWorker myWorker;
  @NotNull private final BitbucketProjectSettings myProjectSettings;
  @NotNull private static final CreateRemoteDoNotAskOption ourDoNotAskOption = new CreateRemoteDoNotAskOption();

  public BitbucketCreatePullRequestDialog(@NotNull final Project project, @NotNull BitbucketCreatePullRequestWorker worker) {
    super(project, true);
    myWorker = worker;

    myProjectSettings = BitbucketProjectSettings.getInstance(project);
    ...
    myPanel.getForkComboBox().addItemListener(e -> {
      ...
      if (e.getStateChange() == ItemEvent.SELECTED) {
        ...
        if (fork.getRemoteName() == null && !fork.isProposedToCreateRemote()) {
          ...
          switch (BitbucketSettings.getInstance().getCreatePullRequestCreateRemote()) {
            case YES:
              createRemote = true;
              break;
            case NO:
              ...
              break;
            case UNSURE:
              createRemote = BitbucketNotifications.showYesNoDialog(project,
                                                                 "Can't Find Remote",
    ...
    BBRepositoryPath defaultRepo = myProjectSettings.getCreatePullRequestDefaultRepo();
  ...
  @TestOnly
  public void testSetFork(@NotNull BBRepositoryPath forkPath) {
  ...
  private static class CreateRemoteDoNotAskOption implements DoNotAskOption {
    ...
    public void setToBeShown(boolean value, int exitCode) {
      if (value) {
        BitbucketSettings.getInstance().setCreatePullRequestCreateRemote(ThreeState.UNSURE);
      }
      else if (exitCode == DialogWrapper.OK_EXIT_CODE) {
        BitbucketSettings.getInstance().setCreatePullRequestCreateRemote(ThreeState.YES);
      }
      else {
        BitbucketSettings.getInstance().setCreatePullRequestCreateRemote(ThreeState.NO);
      }
    }
