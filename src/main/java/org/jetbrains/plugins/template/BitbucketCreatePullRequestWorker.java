...
import org.jetbrains.plugins.template.api.data.BitbucketBranch;
import org.jetbrains.plugins.template.api.data.BitbucketPullRequestDetailed;
...
import org.jetbrains.plugins.template.exceptions.BitbucketOperationCanceledException;
import org.jetbrains.plugins.template.ui.BitbucketSelectForkDialog;
import org.jetbrains.plugins.template.util.*;
...
public class BitbucketCreatePullRequestWorker {
  ...
  @NotNull private final BitbucketApiRequestExecutor myExecutor;
  ...
  @NotNull private final BBRepositoryPath myPath;
  ...
  @NotNull private BBRepositoryPath mySource;

  @NotNull private final List<ForkInfo> myForks;
  @Nullable private List<BBRepositoryPath> myAvailableForks;

  private BitbucketCreatePullRequestWorker(@NotNull Project project,
                                           @NotNull Git git,
                                           @NotNull GitRepository gitRepository,
                                           @NotNull BitbucketApiRequestExecutor executor,
                                           @NotNull BitbucketServerPath server,
                                           @NotNull BitbucketGitHelper helper,
                                           @NotNull ProgressManager progressManager,
                                           @NotNull BBRepositoryPath path,
  ...
  private void doConfigureRemote(@NotNull ForkInfo fork) {
    ...
    BBRepositoryPath path = fork.getPath();
    ...
    try {
      ...
      BitbucketNotifications.showError(myProject, "Can't add remote", "Failed to add GitHub remote: '" + url + "'. " + e.getMessage());
    }
  }

  private void doAddFork(@NotNull BBRepositoryPath path,
    ...
    try {
      ...
      BitbucketNotifications.showWarning(myProject, "Can't load branches for " + path, e);
    }
  }

  @Nullable
  private ForkInfo doAddFork(@NotNull BitbucketRepo repo, @NotNull ProgressIndicator indicator) {
    BBRepositoryPath path = repo.getFullPath();
    for (ForkInfo fork : myForks) {
      if (fork.getPath().equals(path)) {
        return fork;
      }
  ...
  private void doLoadForksFromSettings(@NotNull ProgressIndicator indicator) {
    BBRepositoryPath savedRepo = BitbucketProjectSettings.getInstance(myProject).getCreatePullRequestDefaultRepo();
    if (savedRepo != null) {
    ...
    for (GitRemote remote : myGitRepository.getRemotes()) {
      for (String url : remote.getUrls()) {
        if (myServer.matches(url)) {
          BBRepositoryPath path = BitbucketUrlUtil.getUserAndRepositoryFromRemoteUrl(url);
          if (path != null) {
  ...
  private void doLoadForksFromGithub(@NotNull ProgressIndicator indicator) throws IOException {
    BitbucketRepoDetailed repo = myExecutor.execute(indicator,
            BitbucketApiRequests.Repos.get(myServer, myPath.getOwner(), myPath.getRepository()));
    if (repo == null) throw new BitbucketConfusingException("Can't find github repo " + myPath.toString());
  ...
  @NotNull
  private List<String> loadBranches(@NotNull final BBRepositoryPath fork, @NotNull ProgressIndicator indicator) throws IOException {
    List<BitbucketBranch> branches = BitbucketApiPagesLoader
      .loadAll(myExecutor, indicator, BitbucketApiRequests.Repos.Branches.pages(myServer, fork.getOwner(), fork.getRepository()));
    return ContainerUtil.map(branches, BitbucketBranch::getName);
  }

  @Nullable
  private String doLoadDefaultBranch(@NotNull final BBRepositoryPath fork, @NotNull ProgressIndicator indicator) throws IOException {
    BitbucketRepo repo = myExecutor.execute(indicator,
            BitbucketApiRequests.Repos.get(myServer, fork.getOwner(), fork.getRepository()));
    if (repo == null) throw new BitbucketConfusingException("Can't find github repo " + fork.toString());
  ...
  public DiffInfo getDiffInfo(@NotNull final BranchInfo branch) throws IOException {
    ...
    try {
      ...
      throw new BitbucketOperationCanceledException(e);
    }
  ...
  @Nullable
  private BitbucketPullRequestDetailed doCreatePullRequest(@NotNull ProgressIndicator indicator,
                                                           @NotNull final BranchInfo branch,
                                                           @NotNull final String title,
                                                           @NotNull final String description) {
    final BBRepositoryPath forkPath = branch.getForkInfo().getPath();
    ...
    try {
      return myExecutor.execute(indicator,
              BitbucketApiRequests.Repos.PullRequests
                                  .create(myServer, forkPath.getOwner(), forkPath.getRepository(), title, description, head, base));
    }
    catch (IOException e) {
      BitbucketNotifications.showError(myProject, CANNOT_CREATE_PULL_REQUEST, e);
      return null;
    }
  }
    ...
    return myProgressManager.runProcessWithProgressSynchronously(() -> {
      ...
      try {
        ...
        if (localCommit.getParents().contains(targetCommit.getId())) {
          return BitbucketUtil.getGithubLikeFormattedDescriptionMessage(localCommit.getFullMessage());
        }
        return getSimpleDefaultDescriptionMessage(branch);
      }
        ...
        BitbucketNotifications.showWarning(myProject, "Can't collect additional data", e);
        return getSimpleDefaultDescriptionMessage(branch);
      }
  ...
  public boolean checkAction(@Nullable final BranchInfo branch) {
    if (branch == null) {
      BitbucketNotifications.showWarningDialog(myProject, CANNOT_CREATE_PULL_REQUEST, "Target branch is not selected");
      return false;
    }
    ...
    try {
      info = myProgressManager.runProcessWithProgressSynchronously(
        () -> BitbucketUtil.runInterruptable(myProgressManager.getProgressIndicator(), () -> getDiffInfo(branch)),
        "Collecting Diff Data...", false, myProject);
    }
    catch (IOException e) {
      BitbucketNotifications.showError(myProject, "Can't collect diff data", e);
      return true;
    }
    if (info == null) {
      return true;
  ...
  public void createPullRequest(@NotNull final BranchInfo branch,
    ...
    new Task.Backgroundable(myProject, "Creating Pull Request...") {
      ...
      public void run(@NotNull ProgressIndicator indicator) {
        ...
        if (!result.success()) {
          BitbucketNotifications.showError(BitbucketCreatePullRequestWorker.this.myProject, CANNOT_CREATE_PULL_REQUEST,
        ...
        BitbucketPullRequestDetailed request = doCreatePullRequest(indicator, branch, title, description);
        if (request == null) {
          return;
        }

        BitbucketNotifications.showInfoURL(BitbucketCreatePullRequestWorker.this.myProject, "Successfully created pull request",
  ...
  @Nullable
  private List<BBRepositoryPath> getAvailableForks(@NotNull ProgressIndicator indicator) {
    try {
      List<BitbucketRepo> forks = BitbucketApiPagesLoader
        .loadAll(myExecutor, indicator, BitbucketApiRequests.Repos.Forks.pages(myServer, mySource.getOwner(), mySource.getRepository()));
      List<BBRepositoryPath> forkPaths = ContainerUtil.map(forks, BitbucketRepo::getFullPath);
      if (!forkPaths.contains(mySource)) return ContainerUtil.append(forkPaths, mySource);
      return forkPaths;
    }
    catch (IOException e) {
      BitbucketNotifications.showWarning(myProject, "Can't load available forks", e);
      return null;
    }
  }
  ...
  public void showDiffDialog(@Nullable final BranchInfo branch) {
    if (branch == null) {
      BitbucketNotifications.showWarningDialog(myProject, "Can't Show Diff", "Target branch is not selected");
      return;
    }
    ...
    if (info == null) {
      BitbucketNotifications.showErrorDialog(myProject, "Can't Show Diff", "Can't collect diff data");
      return;
    }
  ...
  public ForkInfo showTargetDialog() {
    ...
    BitbucketSelectForkDialog dialog = new BitbucketSelectForkDialog(myProject, myAvailableForks, getForkPath);
  ...
  @Nullable
  public static BitbucketCreatePullRequestWorker create(@NotNull final Project project,
                                                        @NotNull GitRepository gitRepository,
    ...
    return progressManager.runProcessWithProgressSynchronously(() -> {
      ...
      BBRepositoryPath path = BitbucketUrlUtil.getUserAndRepositoryFromRemoteUrl(remoteUrl);
      if (path == null) {
        BitbucketNotifications.showError(project, CANNOT_CREATE_PULL_REQUEST, "Can't process remote: " + remoteUrl);
        return null;
      }

      GitLocalBranch currentBranch = gitRepository.getCurrentBranch();
      if (currentBranch == null) {
        BitbucketNotifications.showError(project, CANNOT_CREATE_PULL_REQUEST, "No current branch");
        return null;
      }

      BitbucketCreatePullRequestWorker worker =
        new BitbucketCreatePullRequestWorker(project, git, gitRepository, executor, server,
                BitbucketGitHelper.getInstance(), progressManager, path, remote.getName(), remoteUrl,
      ...
      try {
        ...
        BitbucketNotifications.showError(project, CANNOT_CREATE_PULL_REQUEST, e);
        return null;
      }
  ...
  private ForkInfo findRepositoryByUser(@NotNull final ProgressIndicator indicator, @NotNull final String user) {
    ...
    try {
      BitbucketRepo repo;
      BitbucketRepoDetailed target = myExecutor.execute(indicator, BitbucketApiRequests.Repos.get(myServer, user, mySource.getRepository()));

      if (target != null && target.getSource() != null && StringUtil.equals(target.getSource().getUserName(), mySource.getOwner())) {
        repo = target;
      }
      else {
        repo = BitbucketApiPagesLoader
          .find(myExecutor, indicator, BitbucketApiRequests.Repos.Forks.pages(myServer, mySource.getOwner(), mySource.getRepository()),
      ...
      BitbucketNotifications.showError(myProject, "Can't find repository", e);
      return null;
    }
  }
  ...
  public static class ForkInfo {
    ...
    public ForkInfo(@NotNull BBRepositoryPath path, @NotNull List<String> branches, @Nullable String defaultBranch) {
      myPath = path;
    ...
    @NotNull
    public BBRepositoryPath getPath() {
      return myPath;
    }
