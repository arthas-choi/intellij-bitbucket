...
package org.jetbrains.plugins.template.pullrequest.config
...
@State(name = "BitbucketPullRequestsUISettings", storages = [Storage(StoragePathMacros.WORKSPACE_FILE)])
class BitbucketPullRequestsProjectUISettings : PersistentStateComponentWithModificationTracker<BitbucketPullRequestsProjectUISettings.SettingsState> {
  ...
  override fun loadState(state: BitbucketPullRequestsProjectUISettings.SettingsState) {
    this.state = state
  }

  companion object {
    @JvmStatic
    fun getInstance(project: Project) = project.service<BitbucketPullRequestsProjectUISettings>()
