...
class BitbucketResponsePage<T> constructor(val items: List<T>,
  ...
  companion object {
    ...
    @JvmStatic
    @Throws(BitbucketConfusingException::class)
    fun <T> parseFromHeader(items: List<T>, linkHeaderValue: String?): BitbucketResponsePage<T> {
      if (linkHeaderValue == null) return BitbucketResponsePage(items)
      ...
      if (split.isEmpty()) throw BitbucketConfusingException("Can't determine total items count from header: $linkHeaderValue")
      ...
      return BitbucketResponsePage(items, firstLink, prevLink, nextLink, lastLink)
    }

    fun <T> empty(nextLink: String? = null) = BitbucketResponsePage<T>(emptyList(), nextLink = nextLink)
  }
}
