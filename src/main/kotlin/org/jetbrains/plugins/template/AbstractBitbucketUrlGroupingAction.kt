...
/**
 * Visible and enabled if there's at least one possible github remote url ([BitbucketGitHelper]).
 * If there's only one url - it will be used for action, otherwise child actions will be created for each url.
 */
abstract class AbstractBitbucketUrlGroupingAction(text: String?, description: String?, icon: Icon?)
  ...
  protected open fun isEnabledAndVisible(e: AnActionEvent): Boolean {
    ...
    return service<BitbucketGitHelper>().havePossibleRemotes(project)
  }
  ...
  final override fun getChildren(e: AnActionEvent?): Array<AnAction> {
    ...
    val coordinates = service<BitbucketGitHelper>().getPossibleRemoteUrlCoordinates(project)

    return if (coordinates.size > 1) {
      coordinates.map {
        object : DumbAwareAction(it.remote.name + ": " + BitbucketUrlUtil.removeProtocolPrefix(it.url)) {
