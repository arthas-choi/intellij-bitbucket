...
import org.jetbrains.plugins.template.pullrequest.BBPRAccountsComponent
...
object BitbucketUIUtil {
  ...
  fun createIssueLabelLabel(label: BBLabel): JBLabel = JBLabel(" ${label.name} ", UIUtil.ComponentStyle.SMALL).apply {
    background = getLabelBackground(label)
    foreground = getLabelForeground(background)
  }.andOpaque()

  fun getLabelBackground(label: BBLabel): JBColor {
  ...
  fun findAndSelectGitHubContent(project: Project, select: Boolean) {
    ...
    val component = ContentUtilEx.findContentComponent(manager) { c ->
      return@findContentComponent c is BBPRAccountsComponent
    } ?: return
  ...
  sealed class SelectionListCellRenderer<T> : ListCellRenderer<SelectableWrapper<T>>, BorderLayoutPanel() {
    ...
    class PRReviewers(private val iconsProvider: CachingBitbucketAvatarIconsProvider)
      : SelectionListCellRenderer<BBPullRequestRequestedReviewer>() {
      override fun getText(value: BBPullRequestRequestedReviewer) = value.shortName
      override fun getIcon(value: BBPullRequestRequestedReviewer) = iconsProvider.getIcon(value.avatarUrl)
    }

    class Users(private val iconsProvider: CachingBitbucketAvatarIconsProvider)
      : SelectionListCellRenderer<BBUser>() {
      override fun getText(value: BBUser) = value.login
      override fun getIcon(value: BBUser) = iconsProvider.getIcon(value.avatarUrl)
    }

    class Labels : SelectionListCellRenderer<BBLabel>() {
      override fun getText(value: BBLabel) = value.name
      override fun getIcon(value: BBLabel) = ColorIcon(16, ColorUtil.fromHex(value.color))
    }
  }
}
