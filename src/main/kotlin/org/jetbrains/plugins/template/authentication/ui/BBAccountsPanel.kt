...
import org.jetbrains.plugins.template.pullrequest.avatars.CachingBitbucketAvatarIconsProvider
import org.jetbrains.plugins.template.pullrequest.avatars.BBAvatarIconsProvider
...
internal class GHAccountsPanel(private val project: Project,
                               ...
                               private val avatarLoader: CachingBitbucketUserAvatarLoader,
                               private val imageResizer: BitbucketImageResizer) : BorderLayoutPanel(), Disposable {
  ...
  private var currentTokensMap = mapOf<BitbucketAccount, String?>()
  private val newTokensMap = mutableMapOf<BitbucketAccount, String>()
  ...
  private fun addAccount() {
    val dialog = BitbucketLoginDialog(executorFactory, project, this, ::isAccountUnique)
    if (dialog.showAndGet()) {
      val githubAccount = BitbucketAccountManager.createAccount(dialog.getLogin(), dialog.getServer())
  ...
  private fun editAccount(decorator: GithubAccountDecorator) {
    val dialog = BitbucketLoginDialog(executorFactory, project, this).apply {
  ...
  private fun isAccountUnique(login: String, server: BitbucketServerPath) =
  ...
  private fun loadAccountDetails(accountData: GithubAccountDecorator) {
    ...
    progressManager.run(object : Task.Backgroundable(project, "Not Visible") {
      lateinit var loadedDetails: BitbucketAuthenticatedUser
      var correctScopes: Boolean = true

      override fun run(indicator: ProgressIndicator) {
        val (details, scopes) = BBSecurityUtil.loadCurrentUserWithScopes(executor, indicator, account.server)
        loadedDetails = details
        correctScopes = BBSecurityUtil.isEnoughScopes(scopes.orEmpty())
      }

      override fun onSuccess() {
        accountListModel.contentsChanged(accountData.apply {
          details = loadedDetails
          iconProvider = CachingBitbucketAvatarIconsProvider(avatarLoader, imageResizer, executor, BitbucketUIUtil.avatarSize, accountList)
          if (correctScopes) {
            errorText = null
      ...
      override fun onThrowable(error: Throwable) {
        accountListModel.contentsChanged(accountData.apply {
          ...
          showReLoginLink = error is BitbucketAuthenticationException
        })
      }
    })
  }
  ...
  fun setAccounts(accounts: Map<BitbucketAccount, String?>, defaultAccount: BitbucketAccount?) {
   ...
   */
  fun getAccounts(): Pair<Map<BitbucketAccount, String?>, BitbucketAccount?> {
  ...
  fun isModified(accounts: Set<BitbucketAccount>, defaultAccount: BitbucketAccount?): Boolean {
 ...
 */
private class GithubAccountDecorator(val account: BitbucketAccount, var projectDefault: Boolean) {
  var details: BitbucketAuthenticatedUser? = null
  var iconProvider: BBAvatarIconsProvider? = null
