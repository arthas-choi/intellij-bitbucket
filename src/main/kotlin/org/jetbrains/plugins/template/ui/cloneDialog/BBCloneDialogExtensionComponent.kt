...
internal class BBCloneDialogExtensionComponent(
        ...
        private val apiExecutorFactory: BitbucketApiRequestExecutor.Factory,
  ...
  private val repositoryList: JBList<BBRepositoryListItem>
  ...
  private val errorsByAccount = hashMapOf<GithubAccount, BBRepositoryListItem.Error>()
  private val originListModel = CollectionListModel<BBRepositoryListItem>()
  ...
  init {
    val listWithSearchBundle = ListWithSearchComponent(originListModel,
                                                       BBRepositoryListCellRenderer(authenticationManager))

    repositoryList = listWithSearchBundle.list
    val mouseAdapter = BBRepositoryMouseAdapter(repositoryList)
    ...
    if (authenticationManager.hasAccounts()) {
      switchToRepositories()
      authenticationManager.getAccounts().forEach(this@BBCloneDialogExtensionComponent::addAccount)
    }
    else {
      switchToLogin()
    }
  ...
  private fun addAccount(account: GithubAccount) {
    ...
    try {
      ...
      errorsByAccount[account] = BBRepositoryListItem.Error(account,
  ...
  private fun loadUserDetails(account: GithubAccount,
                              executor: BitbucketApiRequestExecutor.WithTokenAuth) {
  ...
  private fun refillRepositories() {
    ...
    for (account in authenticationManager.getAccounts()) {
      ...
      for (repo in repos) {
        originListModel.add(BBRepositoryListItem.Repo(account, user, repo))
      }
    }
  ...
  private fun updateSelectedUrl() {
    ...
    val selectedValue = repositoryList.selectedValue
    if (selectedValue is BBRepositoryListItem.Repo) {
